...
def set_guest_from_session(intent, session, drink_name):
    session_attributes = create_chosen_drink_attributes(drink_name)
    reprompt_text = "Who is speaking again"
    card_title = "Find User"
    should_end_session = False
    if intent ['slots']['guestName']['resolutions']['resolutionsPerAuthority'][0]['status']['code'] == "ER_SUCCESS_MATCH":
        guest_name = intent['slots']['guestName']['value']
        speech_output = "Thanks " + guest_name + " I'll get you a " + drink_name
    else:
        speech_output = "Something has gone wrong with the user add"    
    return build_response(session_attributes, build_speechlet_response(
        card_title, speech_output, reprompt_text, should_end_session))
# --------------- Events ------------------
...
def on_intent(intent_request, session):
    ...
    elif intent_name == "SetGuestNameIntent":
        return set_guest_from_session(intent, session, session['attributes']['chosenDrink'])
    else:
